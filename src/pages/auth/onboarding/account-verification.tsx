...
const AccountVerification = () => {
  ...
  const resendOtp = async () => {
    if (!emailEnabled) return;
    await sendOtp({ email: preAuth.email })
      .unwrap()
      .then(() => {
        setVisible(true);
        resetCountdown();
        ...
        toaster.create({
          ...
          description: `OTP code has been sent to ${preAuth.email}`,
          type: "success",
          duration: 9000,
          closable: true,
        });
    ...
    <GuestLayout>
      ...
      >
        ...
        <VStack w={{ base: "full", md: "4/6" }} spaceY={4}>
          <Progress.Root defaultValue={25} w="full">
            <HStack gap="5">
              <Progress.Track bg="mainBlack" rounded={"md"} flex="1">
                <Progress.Range />
              </Progress.Track>
              <Progress.ValueText color="text_primary">25%</Progress.ValueText>
            </HStack>
          </Progress.Root>
          <VStack w="full" alignItems={"center"}>
            ...
            <Text
              ...
              textAlign={"center"}
            >
              Input the code sent to {preAuth.email} to verify your email
            </Text>
          </VStack>
          <Box w="full">
            {showToast && (
